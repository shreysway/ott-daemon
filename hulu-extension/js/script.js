...
var fields = {
  email: '.hulu-login .hulu-login-input[name="email"]',
  password: '.hulu-login .hulu-login-input[name="password"]',
  submit: '.hulu-login button.login-button'
};
...
$.getJSON(chrome.extension.getURL('config.json'), function (settings) {
  ...
  socket = io(server.protocol + '://' + server.host + ':' + server.port + '/hulu', {
